from fastapi import APIRouter, Depends, status
from sqlalchemy.orm import Session
from app.api.deps import get_current_user, get_db, get_financial_year
from app.db.models import User, FinancialYear
from app.schemas.consumption import ConsumptionEntryCreate, ConsumptionEntryOut
from app.services.consumption_service import create_consumption as create_consumption_service

router = APIRouter()

@router.post("/create_consumption", response_model=ConsumptionEntryOut, status_code=status.HTTP_201_CREATED)
def create_consumption(payload: ConsumptionEntryCreate, db: Session = Depends(get_db), current_user: User = Depends(get_current_user), financial_year: FinancialYear = Depends(get_financial_year)):
    return create_consumption_service(payload, db, current_user, financial_year)

