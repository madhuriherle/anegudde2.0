...
from app.db.models import PurchaseEntry, User
...
def purchases_report(from_date: date = Query(...), to_date: date = Query(...), group_by: str = Query("day"), db: Session = Depends(get_db), _: User = Depends(get_current_user)):
    ...
    rows = (
        ...
        .filter(PurchaseEntry.purchase_date >= from_date, PurchaseEntry.purchase_date <= to_date)
        .group_by(period)
        .order_by(period)
        .all()
    )
