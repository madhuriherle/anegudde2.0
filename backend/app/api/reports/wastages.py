...
@router.get("/wastages", response_model=list[ReportRow])
def wastages_report(from_date: date = Query(...), to_date: date = Query(...), group_by: str = Query("day"), db: Session = Depends(get_db), _: User = Depends(get_current_user)):
    ...
    rows = (
        ...
        .filter(WastageEntry.wastage_date >= from_date, WastageEntry.wastage_date <= to_date)
        .group_by(period)
        .order_by(period)
        .all()
    )
