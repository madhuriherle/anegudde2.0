...
def create_adjustment(payload: StockAdjustmentCreate, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    next_adjustment_id = (db.query(func.max(StockAdjustment.id)).scalar() or 0) + 1
    adjustment = StockAdjustment(id=next_adjustment_id, **payload.model_dump(), user_id=current_user.id, created_at=now, created_by=current_user.id)
    ...
    db.add(StockLedger(item_id=item.id, txn_date=payload.adjustment_date, txn_type=4, ref_table="stock_adjustments", ref_id=adjustment_id, qty_in=payload.adjusted_qty if payload.adjusted_qty > 0 else 0, qty_out=abs(payload.adjusted_qty) if payload.adjusted_qty < 0 else 0, unit_cost=item.default_price or 0, value_in=(payload.adjusted_qty * (item.default_price or 0)) if payload.adjusted_qty > 0 else 0, value_out=(abs(payload.adjusted_qty) * (item.default_price or 0)) if payload.adjusted_qty < 0 else 0, balance=item.current_stock, created_at=now, updated_at=now, created_by=current_user.id, updated_by=current_user.id))
    db.commit()
