...
from app.db.models import Item, StockAdjustment, StockLedger, User, FinancialYear
...
def create_adjustment(payload: StockAdjustmentCreate, db: Session = Depends(get_db), current_user: User = Depends(get_current_user), financial_year: FinancialYear = Depends(get_financial_year)):
    ...
    adjustment = StockAdjustment(**payload.model_dump(), financial_year_id=fy_id, user_id=current_user.id, created_at=now, created_by=current_user.id)
