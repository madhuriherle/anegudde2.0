...
const MonthlyPerformanceReportPage = () => {
  ...
  const totalStockValue = monthlyData?.reduce((sum, row) => sum + (Number(row.stock_value) || 0), 0) || 0;
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      const link = document.createElement('a');
      link.href = url;
      link.setAttribute('download', 'monthly_performance_report.pdf');
      document.body.appendChild(link);
      link.click();
      link.remove();
