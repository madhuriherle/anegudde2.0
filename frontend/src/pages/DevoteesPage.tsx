...
const DevoteesPage: React.FC = () => {
  ...
  const columns = useMemo<ColumnDef<any>[]>(() => [
    ...
    },
    {
      ...
      header: 'Email',
      size: 220,
      cell: info => <span className="text-text-normal">{(info.getValue() as string) || '-'}</span>,
    },
    {
      accessorKey: 'address',
      header: 'Address',
      size: 260,
      cell: info => (
        <span className="block max-w-[260px] truncate text-text-normal" title={(info.getValue() as string) || '-'}>
    ...
    },
    {
      ...
      header: 'Last Updated',
      size: 140,
      cell: info => <span className="text-text-normal">{formatDate(info.getValue() as string)}</span>,
    },
    {
      id: 'actions',
                  ...
                  <table className="w-full min-w-[760px] table-fixed text-left text-sm">
                    ...
                    <tbody className="divide-y divide-[#f0e5da]">
                      {devoteeDetails.donations?.length ? (
                        devoteeDetails.donations.map((donation: any) => (
