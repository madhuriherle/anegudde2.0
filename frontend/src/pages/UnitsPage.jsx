...
const UnitsPage = () => {
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id) => api.delete(`/units/delete_unit/${id}`),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['units'] });
  ...
  const statusMutation = useMutation({
    mutationFn: async ({ id, status }) => api.put(`/units/update_unit/${id}`, { status }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['units'] });
  ...
  const handleOpen = (unit = null) => {
    setEditingUnit(unit);
    if (unit) reset(unit);else
    reset({ unit_name: '', unit_code: '', status: 1 });
    setOpen(true);
  };
  ...
  },
  {
    ...
    <InlineStatusSelect
      ...
      onChange={async (nextStatus) => {
        const confirmed = await showConfirm(
          'Update Status',
          `Are you sure you want to ${Number(nextStatus) === 1 ? 'activate' : 'deactivate'} "${info.row.original.unit_name}"?`
        );
