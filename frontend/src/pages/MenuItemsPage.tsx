...
const MenuItemsPage: React.FC = () => {
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: number) => api.delete(`/menu-items/delete_menu_item/${id}`),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['menu-items'] });
  ...
  const statusMutation = useMutation({
    mutationFn: async ({ id, status }: { id: number; status: number }) => api.put(`/menu-items/update_menu_item/${id}`, { status }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['menu-items'] });
  ...
  const columns = useMemo<ColumnDef<any>[]>(() => [
    ...
    },
    {
      ...
      size: 150,
      cell: info => {
        const unit = unitOptions.find((u: any) => u.id === info.getValue());
