...
class PrintingService {
  // The installed printer list never changes while the app is running, so
  // querying the OS for it on every single print (which the fallback path
  // used to do) just adds avoidable delay - query it once per app session
  // and reuse that same list from then on. A newly added printer only
  // shows up after the app is restarted, which is expected in that case.
  static List<Printer>? _cachedPrinters;

  static Future<List<Printer>> _listPrintersCached() async {
  ...
  /// Pays the one-time costs (OS printer enumeration, Kannada font load)
  /// up front in the background at app startup, so the very first actual
  /// token print doesn't have to pay them - call this once, unawaited,
  /// right when the app launches.
  static Future<void> warmUp() async {
    try {
      await _listPrintersCached();
    } catch (_) {
      // Fine - the real print call will just try again later.
    }
    try {
      // Force the Kannada font's glyphs to load now by building (and
      // immediately discarding) a throwaway paragraph with it - nothing is
      // drawn or shown, this just touches the font loading machinery.
      ...
      final builder = ui.ParagraphBuilder(style)
        ..pushStyle(ui.TextStyle(fontFamily: 'KannadaFont', fontSize: 21))
        ..addText(_templeNameLine1);
      builder.build().layout(const ui.ParagraphConstraints(width: 400));
    } catch (_) {
      // Fine - the real print call will just render it fresh later.
    }
  }
  ...
  static Future<void> printToken(
    ...
    try {
      final printers = await _listPrintersCached();
      ...
      // layoutPdf shows a native print dialog requiring manual interaction
      // (unlike directPrintPdf, which prints silently) - logging clearly
      // here so a "print seemed to hang" report can be traced back to this
      // specific path instead of looking like an unexplained freeze.
      print(
        'Direct print failed, falling back to layoutPdf (this will show a '
        'print dialog requiring manual confirmation): $e',
      );
