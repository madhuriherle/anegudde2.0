...
def main():
    ...
    print("--- Uploading update_listed_items.py to VPS ---")
    sftp.put(os.path.join(local_base, "update_listed_items.py"), f"{remote_base}/update_listed_items.py")

    print("--- Running update script on VPS ---")
    run_command(ssh, f"cd {remote_base} && backend/.venv/bin/python update_listed_items.py")

    sftp.close()
    ssh.close()
    print("VPS update execution complete.")
